...
describe('MsgExecuteContractProtobuf', () => {
    ...
    beforeEach(async () => {
        ...
        const deployResult = await MsgExecuteContractProtobufEncode.sendDeploy(deployer.getSender(), toNano('0.05'));

        expect(deployResult.transactions).toHaveTransaction({
            from: deployer.address,
            to: MsgExecuteContractProtobufEncode.address,
            deploy: true,
            success: true,
        });
    });

    it('test MsgExecuteContractProtobuf encode', async () => {
