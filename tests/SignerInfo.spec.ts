...
describe('Signer Info', () => {
    ...
    let signerInfo: SandboxContract<LightClient>;
    ...
    it('test encode', async () => {
        ...
        const contractResult = await signerInfo.getSignerInfoEncode(data[0]);
        ...
        const modeInfo = contractResult.readCell();
        buffer = Buffer.concat([buffer, Buffer.from(modeInfo.bits.toString(), 'hex')]);
        const sequence = contractResult.readCell();
        buffer = Buffer.concat([buffer, Buffer.from(sequence.bits.toString(), 'hex')]);
