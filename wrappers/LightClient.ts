...
export const buildTxBodySliceRef = (txBodyWasm: TxBody): Cell => {
    ...
    for (let i = txBodyWasm.messages.length - 1; i >= 0; i--) {
        ...
        const cell = beginCell()
            .storeRef(beginCell().storeBuffer(Buffer.from(messages[i].typeUrl)).endCell())
            .storeRef(value ? value : beginCell().endCell())
            .endCell();
        if (!messagesRef) {
            messagesRef = beginCell().storeSlice(cell.beginParse()).storeRef(beginCell().endCell()).endCell();
        } else {
            messagesRef = beginCell().storeSlice(cell.beginParse()).storeRef(messagesRef).endCell();
        }
    }
    ...
    for (let i = ext_opts.length - 1; i >= 0; i--) {
        ...
        const cell = beginCell()
            .storeRef(beginCell().storeBuffer(Buffer.from(ext_opts[i].typeUrl)).endCell())
            .storeRef(value ? value : beginCell().endCell())
            .endCell();
        if (!extensionOptionsRef) {
            extensionOptionsRef = beginCell().storeSlice(cell.beginParse()).storeRef(beginCell().endCell()).endCell();
        } else {
            extensionOptionsRef = beginCell().storeSlice(cell.beginParse()).storeRef(extensionOptionsRef).endCell();
        }
    }
    ...
    for (let i = non_critical_ext_opts.length - 1; i >= 0; i--) {
        ...
        const cell = beginCell()
            .storeRef(beginCell().storeBuffer(Buffer.from(non_critical_ext_opts[i].typeUrl)).endCell())
            .storeRef(value ? value : beginCell().endCell())
            .endCell();
